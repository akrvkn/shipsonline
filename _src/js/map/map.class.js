...
function Map (selector, options) {
	...
	me.routes = [];
    /*
     * Map tourlines will be stored in array
     */
    me.tourlines = [];
	/*
 ...
 * @param {boolean} enable Enable/disable map autozoom
 */
Map.prototype.polyZoom = function (enable) {
    ...
    this.tourlines.map(function (polyline) {
        for (var i = 0; i < polyline.path.length; i++) {
            bounds.extend(
                new google.maps.LatLng(polyline.path[i].lat, polyline.path[i].lng)
            );
        }
...
Map.prototype.addMarker = function (options) {
	...
	var result = new MapTour.Map.Marker(options);
	result.setMap(this);
	return result;
...
Map.prototype.addRoute = function (markerStart, markerStop, options) {
	...
	var result = new MapTour.Map.Route(markerStart, markerStop, options);
	result.setMap(this);
	return result;
...
/**
 * Add new route point-to-point to map
 * @param {Marker} markersArray  - route points
 * @param {object} options Route options
 */
Map.prototype.addTourline = function (markersArray, options) {

    var result = new MapTour.Map.Tourline(markersArray, options);
    result.setMap(this);
    return result;
...
/**
 * Clear map and removing all polylines
 */
Map.prototype.clearTourlines = function () {

    this.tourlines.map(function (line) {
